...
public class Filter implements QueryParameters {
	public enum MatchingOperator {
		...
		MatchingOperator(final String representation, final boolean unary) {
			...
			encoded = representation.replace(" ", SPACE);
			this.unary = unary;
		}
	...
	public boolean equals(final Object o) {
		...
		return (positive == filter.positive) && Objects.equals(children, filter.children) && (boolOp == filter.boolOp)
	...
	public String toEncodedString() {
		...
		if (leaf == null) {
			final String result = children.stream().map(Filter::toEncodedString)
					.collect(Collectors.joining(SPACE + boolOp + SPACE));
			if ((parent == null) && positive) {
				return result;
			}
			return prefix + "(" + result + ")";
		...
		if ((leaf.matchingOperator == MatchingOperator.IN) || (leaf.matchingOperator == MatchingOperator.NOT_IN)) {
			return prefix + DOUBLEQUOTE + leaf.value + DOUBLEQUOTE + SPACE + leaf.matchingOperator.encoded + SPACE
					+ leaf.selector;
		}
		return prefix + leaf.selector + SPACE + leaf.matchingOperator.encoded + SPACE + DOUBLEQUOTE + leaf.value
				+ DOUBLEQUOTE;
	}
	...
	private static class Leaf {
		...
		public boolean equals(final Object o) {
			...
			return (matchingOperator == leaf.matchingOperator) && selector.equals(leaf.selector)
